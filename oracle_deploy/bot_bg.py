...
def parsear_sessao_bg(soup):
    ...
    try:
        ...
        for div_info in soup.find_all("div", class_="cb-progress-info"):
            ...
            max_s = m.group(2).replace(",", ".")
            if "." in val_s:
                continue  # ignora dias gastos (valor decimal)
            val, maximo = int(val_s), int(max_s)
            if maximo == 100:
                if "batalhas_dia" not in sessao:
                    sessao["batalhas_dia"]     = val
                    sessao["batalhas_dia_max"] = maximo
                else:
                    sessao["batalhas_total"]  = maximo
                    sessao["batalhas_feitas"] = val
            elif maximo in (200, 300, 400):
                sessao["batalhas_total"]  = maximo
                sessao["batalhas_feitas"] = val

        # ── Fallback via regex de texto (PT/DE/PL e variantes EN antigas) ──
        if "batalhas_dia" not in sessao:
            _PATTERNS_MAX = [
                r"(\d+)\s+de\s+um\s+m[aá]ximo\s+de\s+(\d+)",          # PT
                r"(\d+)\s+out\s+of\s+a\s+maximum\s+(\d+)",             # EN real
                r"(\d+)\s+of\s+a\s+maximum\s+of\s+(\d+)",              # EN alt
                r"(\d+)\s+of\s+(?:the\s+)?maximum\s+(\d+)",            # EN alt2
                r"(\d+)\s+von\s+maximal\s+(\d+)",                      # DE
                r"(\d+)\s+z\s+maksymalnie\s+(\d+)",                    # PL
                r"(\d+)\s*/\s*(\d+)\s*(?:battle|Kampf|walka|boj)",     # genérico
            ]
            matches = []
        ...
        _DATA_INICIO = [
            ...
            r"session\s+begins[^:]*:\s*on\s+(\d{4}-\d{2}-\d{2})\s+at\s+(\d{2}:\d{2}:\d{2})",  # EN real
            r"Session\s+start[^:]*:\s*([\d.]+\s[\d:]+)",               # EN alt
        ...
        for pat in _DATA_INICIO:
            ...
            if m:
                sessao["inicio"] = f"{m.group(1)} {m.group(2)}" if m.lastindex == 2 else m.group(1)
                break

        _DATA_FIM = [
            r"Fim da sess[aã]o[^:]*:\s*([\d.]+\s[\d:]+)",              # PT
            r"(?:ending\s+time|latest\s+ending)[^:]*:\s*on\s+(\d{4}-\d{2}-\d{2})\s+at\s+(\d{2}:\d{2}:\d{2})",  # EN real
            r"Session\s+end[^:]*:\s*([\d.]+\s[\d:]+)",                 # EN alt
        ...
        for pat in _DATA_FIM:
            m = re.search(pat, texto, re.IGNORECASE)
            if m:
                sessao["fim"] = f"{m.group(1)} {m.group(2)}" if m.lastindex == 2 else m.group(1)
                break

        # Restantes multilíngua (override do calculado se a página informa explicitamente)
        _RESTANTES = [
            r"pode realizar mais\s+(\d+)\s+ataques",                    # PT
            r"no\s+more\s+than\s+a\s+total\s+of\s+(\d+)\s+attacks?",  # EN real
            r"can\s+(?:still\s+)?perform\s+(?:up\s+to\s+)?(\d+)\s+attack",  # EN alt
